...
public class ApplicationPreferences extends AbstractPreferences {
	 ...
	 * � l'application, se trouvant g�n�ralement dans le r�pertoire de l'application et celui
	 * de l'utilisateur, qui r�side en principe dans le dossier de l'utilisateur
	 */
	private Properties defaultProperties, applicationProperties, userProperties;
	/**
	 * Une r�f�rence aux fichiers de preferences utilis� pour les preferences de l'application
	 ...
	 * se trouve � la racine du classpath et se nomme default.config. Le nom et l'emplacement du
	 * fichier peut �tre modifi� grace � la propri�t� system java.util.prefs.PreferencesFactory.defaultFile.
	 * Il est localis� au moyen de ClassLoader.getSystemResource.
	 * 
	 * Le fichier des preferences de l'application se trouve par d�faut � la racine de l'application
	 * et se nomme application.config. L'emplacement et le nom du fichier peuvent �tre modifi�
	 * en utilisant la preference applicationConfigFile qui doit se trouver dans le fichier de config
	 * par defaut.
	 * 
	 * Le fichier utilisateur se trouve par d�faut dans le dossier utilisateur et se nomme .unnamedJavaApp.config
	 * L'emplacement et le nom peuvent �tre modifi� grace � la preference userConfigFile, qui peut se
	 * trouv� dans le fichier par defaut ou dans celui de l'application, la preference de l'application
	 * prenant le dessus. Dans le chemin d'acc�s au fichier, la chaine "%user.dir%" sera remplac� par la
	 * propri�t� system user.home (System.getProperty("user.home");
	 * @throws IOException
	 */
